    ...
    try:
        with torch.inference_mode():
            for batch in source.to_batches(batch_size=batch_size, columns=columns):
                prediction = module(torch.from_numpy(_feature_matrix(batch, module.feature_columns))).cpu().numpy()
...
def _feature_matrix(batch: pa.RecordBatch, columns: tuple[str, ...]) -> np.ndarray:
    matrix = np.empty((batch.num_rows, len(columns)), dtype=np.float32)
